...
func generate_obs():
	...
	if active_green.is_empty() or last_green.position.x < $Game.get_node("Camera2D").position.x + randi_range(300,500): #instead of using timer, add obstacle once previous obstacle is psat a random point
		...
		var green_x: int = screen_size.x + speed_change + 100 #+speed_change bc game is const moving to the left; +100 for buffer
		var green_y: int = screen_size.y - ground_height - (green_height*green_scale.y /2) + 5
